...
| TBD | SS-017 — Worker fork trigger ADR | Open | DDD |
...
### SS-007 — How Screwed Am I (live deployment)
...
profiles. Survival probability meter. Attenborough verdict. Panel cards
with death commentary triggers.

Files built. Deployed to Cusslab Worker via /survival-school/assess route.
Testing live now.
...
Prototype already built in this session — needs wiring to live Worker.
...
Session-preflight-fix.md in outputs. Apply to session-startup.md.
...
by uploading files and pasting content. Significant friction. Reported
to Anthropic multiple times.
